...
def rho(x,flag):
    ...
    fg = 0.16
    if(flag==0):
        return fg*M/(2*np.pi) * a/x * 1/(x+a)**3 / 435
    if(flag==1):
        return fg*M/(2*np.pi) * a/x * 1/(x+a)**3

def cdf(x):
    integral, _ = quad(rho, 0.1, x, args=(0,))
    normalization, _ = quad(rho, 0.1, 4, args=(0,))
...
for r in R:
    ...
    theta = np.random.uniform(0, np.pi)    # Angle between 0 and pi (polar angle)
    phi = np.random.uniform(0, 2*np.pi)    # Angle between 0 and 2*pi (azimuthal angle)
...
plt.plot(bin_edges, rho(bin_edges, 0))
plt.show()
