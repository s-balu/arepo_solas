...
ms = np.array([row[0] for row in data])
xs = np.array([row[1] for row in data])
ys = np.array([row[2] for row in data])
zs = np.array([row[3] for row in data])
vxs = np.array([row[4] for row in data])
vys = np.array([row[5] for row in data])
vzs = np.array([row[6] for row in data])
...
N5 = len(ms5)
N4 = len(ms4)
Pos_star5 = np.zeros([N5,3],dtype=FloatType)
Pos_star4 = np.zeros([N4,3],dtype=FloatType)
...
Vel_star5 = np.zeros([N5,3],dtype=FloatType)
Vel_star4 = np.zeros([N4,3],dtype=FloatType)
...
NumPart = np.array([Ntot,0,0,0,N4,N5], dtype=IntType)
...
part5.create_dataset("ParticleIDs" , data=np.arange(Ntot+N4+1,Ntot+N4+1+N5+1))
...
part5.create_dataset("BlackholeHsml" , data=np.full(N5, hsml, dtype=FloatType))
